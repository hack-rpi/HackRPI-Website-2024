...
const PastYearProjects: React.FC = () => {
	const allProjects: ProjectDisplayProps[] = [
		...
		},
		{
			...
			description: "PyPlay ports through the basic features of Scratch via PyGame and handles all of the multithreading behind the scenes so that kids can learn to code easily, continuing making Scratch-like games while learning to write in a full-fledged programming language.",
		...
		},
		{
			...
			description: "In some cases when going to the ER, it may be faster to go to a further away ER to avoid waiting time and be seen faster in net time. Using our tool, we will find you the ERs that will see you the soonest based on a combination of the time to drive to the ER and the estimated ER waiting time.",
			imageUrl: "/projectImages/googlecloudusage2.png",
		},
		{
			title: "GitHub IDE",
			authors: ["Momir-Petrovic Petrovic", "Advait"],
			description: "Our project allows you to run your code straight from the github repository readme file. Whenever the GitHub repository has a commit, it takes the readme, compiles the code and adds the output to the end of the readme",
			imageUrl: "/projectImages/unNamed.png",
		},
		{
			title: "Terra",
			authors: ["Liam Agu Vaitkus", "Bigmaccar McCarthy", "Lillith", "Christian Monfiston"],
			description: 'Terra, named after the goddess of nature, or, "Mother Earth," seeks to connect humanitarian organizations with volunteers who are eager to lend a hand to communities impacted by tragedy. Organizations can post volunteer openings and our site facilitates the rest.',
			imageUrl: "/projectImages/domainName.png",
		},
		{
			title: "SparkSwap",
			authors: ["Joe Schmoe", "Tripp Lyons"],
			description: "SparkSwap allows users to buy electricity tokens either directly from energy companies in advance or purchase from traders. These tokens are for a certain amount of electricity produced in a certain manner at a particular time. These tokens are then redeemed for actual electricity from the company. The ability of a user to directly choose how much power of each particular type they want is a strength of this system. Additionally, buying a particular amount of electricity from the energy company ahead of time reduces the risk for the energy company and the price for the consumer, thus both have an incentive to use the service.",
			imageUrl: "/projectImages/web3.png",
		},
		{
			title: "Graphing Guinea",
			authors: ["Hanna Nakajima", "Kevin Liu", "Claire Chen", "MEKNOWKNOOB Lee"],
			description: "Our game uses puzzle-solving to make learning fun. Each level begins with a guinea pig trying to traverse a gap to the other side of the room. With the player's inputted equations and limits, the guinea pig's tunnel is created allowing it to successfully reach the end goal.",
